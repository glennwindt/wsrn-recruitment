...
export default function OfficeSuppliesTracker({ userRole = "admin" }) {
  ...
  const handleSubmit = (e) => {
    ...
    alert(`✅ ${newPurchase.item} added to tax-deductible items.`);
  ...
  const totalDeductible = purchases
    .filter(p => p.taxDeductible)
    .reduce((sum, p) => sum + parseFloat(p.amount), 0);

  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg max-w-5xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">Office Supplies & Tax Deductibles</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="Shipping">Delivery / Freight</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="E.g., Amazon, CTT Store, Continente"
              required
              className="w-full p-3 bg-gray-800 rounded"
            />
          </div>
      ...
      {/* Purchase List */}
      <section>
        <h3 className="text-xl font-semibold mb-4">Office Purchases ({purchases.length})</h3>
              ...
              {purchases.map((purchase, index) => {
                ...
                return (
                  <tr key={index} className="hover:bg-gray-700 transition border-b border-gray-700">
                    ...
                    <td className="py-3 text-right">€{parseFloat(purchase.amount).toFixed(2)}</td>
                    <td className="py-3 text-right">
                      <span className={`inline-block px-2 py-1 rounded text-xs ${
                        purchase.taxDeductible ? "bg-green-900/30 text-green-400" : "bg-red-900/30 text-red-400"
                      }`}>
                        {purchase.taxDeductible ? "Yes" : "No"}
                      </span>
                    </td>
                    <td className="py-3 text-right">
                      {purchase.cttTracking && (
                        <a href={`https://www.ctt.pt/en/tracking/?trackingCode= ${purchase.cttTracking}`}
                           target="_blank"
                    ...
                    <td className="py-3 text-right space-x-3">
                      <button
                        onClick={() => alert(`Edit ${purchase.id}`)}
                        className="text-blue-400 hover:text-blue-300 text-sm"
                      >
                        🖊 Edit
                      </button>
